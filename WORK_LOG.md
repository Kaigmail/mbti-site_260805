...
## 2026-08-09 匹配算法修复(方案A:消除排序效应)
- 发现:Kai 反馈朋友测出结果趋同 → 随机模拟 10000 次,分布 12.8% 到 5.0%(极差2.5倍)
- 根源:①轨迹强迫/暴躁破防选项偏心(3次) ②平局默认取列表第一个(TURBO 排第一占便宜) ③画像大小不齐
- 修复:matching.js 平局改为答案哈希确定性决胜(同答案同结果,无顺序偏置);index.js 传 seed
- 验证:随机分布变为 11.1% 到 6.6%(极差1.7倍);回归全A/B/C 不变;确定性 OK
- 残余偏差:题目选项偏心(方案B 可再修,待 Kai 决定)

## 2026-08-09 项目收尾
- 状态:全部功能完成,线上已部署(https://y6ndnkcbq4.coze.site),git 15 提交已推送,工作区干净
- 文档:PRD / 实现计划 / V2计划 / README / RETROSPECTIVE / WORK_LOG 齐全
- Obsidian:项目索引(🟢完成)+ 踩坑记录 + 复盘 + 对外教程已同步
- 待办(可选):①线上重新部署 v3(含匹配修复) ②方案B 题目偏心修复 ③推广分享
- 技能沉淀:vibe-coding-workflow(设计稿先行)、codex(Windows 坑)、coze-deployment 均已更新
